...
const tui = async (api) => {
    ...
    const collectEntries = (sessionID, tick) => {
        const entries = [];
        const candidates = [];
        ...
        for (const message of messages) {
            ...
            for (const part of parts) {
                ...
                if (!isAgentToolPart(part.type, part.tool) || !part.id || !part.tool)
                    continue;
                ...
                const status = resolveToolStatus(state.status, childStatus, background && followsChildSession);
                if (!status)
                    continue;
                ...
                candidates.push({
                    ...
                    agent: readString(input.subagent_type) ??
        ...
        for (const entry of deduplicateCandidates(candidates)) {
            const legacyUpdate = entry.childReference
                ...
                : undefined;
            if (legacyUpdate?.description)
                entry.description = legacyUpdate.description;
            if (entry.kind === "background" && entry.parentStatus === "completed") {
                if (entry.followsChildSession &&
                    entry.status === "completed" &&
                    entry.childSessionID) {
                    const child = classifyChildCompletion(api.state.session.messages(entry.childSessionID));
                ...
                if (entry.childReference?.kind === "legacy" && !entry.followsChildSession) {
                    entry.status = resolveLegacyBackgroundStatus(entry.status, legacyUpdate, lifecycle.wasObservedLive(entry.key));
                    if (isLive(entry.status))
                        entry.completedAt = undefined;
                }
            }
            seen.add(entry.key);
            if (lifecycle.retain(entry, tick))
                entries.push(entry);
        }
...
function renderChildren(entries, tickNow, isCollapsed, status, onToggle) {
