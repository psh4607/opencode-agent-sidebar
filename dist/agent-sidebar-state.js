...
export function isAgentToolPart(type, tool) {
    return type === "tool" && (tool === "task" || tool === "delegate");
}
export function isBackgroundInvocation(tool, input, metadata) {
    return (tool === "delegate" ||
        input.background === true ||
        input.run_in_background === true ||
        metadata.background === true);
}
export function resolveChildReference(input, metadata, output) {
    const sessionID = readString(metadata.sessionId) ?? readString(metadata.jobId) ?? readString(input.task_id);
    if (sessionID)
        return { id: sessionID, kind: "session" };
    const legacyID = readString(metadata.backgroundTaskId) ??
...
export function parseLegacyBackgroundUpdates(body) {
    const updates = [];
    ...
    if (body.includes("[ALL BACKGROUND TASKS COMPLETE") || body.includes("[ALL BACKGROUND TASKS FINISHED")) {
        for (const match of body.matchAll(TASK_SUMMARY_LINE_PATTERN)) {
            const id = match[1];
            if (id)
                updates.push([id, { status: "completed", description: match[2] ?? "" }]);
        }
    }
    return updates;
}
export function resolveToolStatus(toolStatus, childStatus, followChildAfterCompletion) {
    if (toolStatus === "pending")
        return "queued";
    if (toolStatus === "running")
        return childStatus === "retry" ? "retrying" : "running";
    if (toolStatus === "error")
        return "error";
    if (toolStatus !== "completed")
        return undefined;
    if (followChildAfterCompletion && childStatus === "busy")
        return "running";
    if (followChildAfterCompletion && childStatus === "retry")
        return "retrying";
    return "completed";
}
export function resolveLegacyBackgroundStatus(status, update, wasObservedLive) {
    if (status !== "completed")
        return status;
...
export function classifyChildCompletion(messages) {
...
export function isVisible(entry, tick) {
    return (isLive(entry.status) ||
...
export function compareEntries(a, b) {
    ...
    if (a.kind !== "main" && b.kind === "main")
        return 1;
    if (isLive(a.status) && !isLive(b.status))
        return -1;
    if (!isLive(a.status) && isLive(b.status))
        return 1;
...
export function deduplicateCandidates(candidates) {
    ...
    for (const entry of candidates) {
        ...
        if (!previous || entry.startedAt >= previous.startedAt)
            newestByChild.set(entry.childSessionID, entry);
    }
...
export function createLifecycleTracker() {
    ...
    return {
        ...
        startTime(key, status, supplied, tick) {
